...
static public class NGUIText
{
    ...
    static public void Prepare(string text, UIFont font)
    {
#if DYNAMIC_FONT
        if (font != null)
        {
            if (font.dynamicFont != null)
                font.dynamicFont.RequestCharactersInTexture(text, finalSize, font.dynamicFontStyle);
        }
#endif
    }
	/// <summary>
    ...
    static public BMSymbol GetSymbol(string text, int index, int textLength, UIFont font)
    {
        BMSymbol nSymbolRet = null;
        if (font != null)
        {
            nSymbolRet = (font != null) ? font.MatchSymbol(text, index, textLength) : null;
        }
        return nSymbolRet;
    }
    ...
    static public float GetGlyphWidth(int ch, int prev, UIFont uifont)
    {
#if DYNAMIC_FONT
        if (uifont.dynamicFont != null)
        {
            if (uifont.dynamicFont.GetCharacterInfo((char)ch, out mTempChar, finalSize, fontStyle))
#if UNITY_4_3 || UNITY_4_5 || UNITY_4_6 || UNITY_4_7
                return mTempChar.width * fontScale * pixelDensity;
#else
                return mTempChar.advance * fontScale * pixelDensity;
        ...
        else if (uifont.bmFont != null)
        {
            ...
            BMGlyph bmg = uifont.bmFont.GetGlyph(ch);
		...
		ref int sub, ref bool bold, ref bool italic, ref bool underline, ref bool strike, ref bool ignoreColor)
	{
		...
		if (text[index + 7] == ']')
		{
			...
			if (colors != null)
			{
                ...
                //	colors.Add(c);
                size = colors.size;
                colors.SetAllocateAddSize(1);
                colors.buffer[size] = c;
            }
			index += 8;
			return true;
		}
    ...
    static public void CalculatePrintedSize2(string text, ref List<int> lineList)
    {
        if (lineList == null)
        {
        ...
        if (!string.IsNullOrEmpty(text))
        {
            ...
            v.x = ((x > maxX) ? x - finalSpacingX : maxX);
            v.y = (y + finalLineHeight);
            lineList.Add(textLength);
        }
        
    }

    static public Vector2 CalculatePrintedSize(string text, UIFont font)
    {
        Vector2 v = Vector2.zero;
        ...
        if (!string.IsNullOrEmpty(text))
        {
            ...
            for (int i = 0; i < textLength; ++i)
            {
                ...
                // See if there is a symbol matching this text
                BMSymbol symbol = useSymbols ? GetSymbol(text, i, textLength, font) : null;



                if (symbol == null)
                {
                    float w = GetGlyphWidth(ch, prev, font);
                    if (w != 0f)
                    {
                        w += finalSpacingX;
	...
	static public bool WrapText (string text, out string finalText, bool keepCharCount, bool wrapLineColors)
	{
		...
		bool fits = true;
		bool eastern = true; //单词不强制合并在一行内，以避免图文混排的计算不准确 by liuzhen 2015-7-29 17:37:51
		...
		if (wrapLineColors) //mColors.Add(c);
        {
            int size;
            size = mColors.size;
            mColors.SetAllocateAddSize(1);
            mColors.buffer[size] = c;
        }
	...
	static public void Print (string text, BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
        ...
        size = mColors.size;
        mColors.SetAllocateAddSize(1);
        mColors.buffer[size] = Color.white;
		...
		for (int i = 0; i < textLength; ++i)
		{
			...
			//	verts.Add(new Vector3(v1x, v0y));
                size = verts.size;
                verts.SetAllocateAddSize(4);
                verts.buffer[size + 0] = new Vector3(v0x, v0y);
                verts.buffer[size + 1] = new Vector3(v0x, v1y);
                verts.buffer[size + 2] = new Vector3(v1x, v1y);
                verts.buffer[size + 3] = new Vector3(v1x, v0y);
				...
				if (uvs != null)
				{
                    ...
                    size = uvs.size;
                    uvs.SetAllocateAddSize(4);
                    uvs.buffer[size + 0] = new Vector2(u0x, u0y);
                    uvs.buffer[size + 1] = new Vector2(u0x, u1y);
                    uvs.buffer[size + 2] = new Vector2(u1x, u1y);
                    uvs.buffer[size + 3] = new Vector2(u1x, u0y);
                }
				...
				if (cols != null)
				{
                    ...
                    else
                    {
                        ...
                        for (int b = 0; b < 4; ++b)
                        {
                            cols.buffer[size++] = new Color32(1, 1, 1, uc.a);
                        }
					}
				}
			}
			else // No symbol present
			{
				...
				if (uvs != null)
				{
					...
					for (int j = 0, jmax = (bold ? 4 : 1); j < jmax; ++j)
					{
                        ...
                        size = uvs.size;
                        uvs.SetAllocateAddSize(4);
                        uvs.buffer[size + 0] = glyph.u0;
                        uvs.buffer[size + 1] = glyph.u1;
                        uvs.buffer[size + 2] = glyph.u2;
                        uvs.buffer[size + 3] = glyph.u3;
                    }
                }
					...
					if (glyph.channel == 0 || glyph.channel == 15)
					{
						if (gradient)
						{
							...
							for (int j = 0, jmax = (bold ? 4 : 1); j < jmax; ++j)
							{
                                ...
                                size = cols.size;
                                cols.SetAllocateAddSize(4);
                                cols.buffer[size + 0] = s_c0;
                                cols.buffer[size + 1] = s_c1;
                                cols.buffer[size + 2] = s_c1;
                                cols.buffer[size + 3] = s_c0;
				...
				if (!bold)
				{
                    ...
                    else // Italic
					{
                        ...
                        size = verts.size;
                        verts.SetAllocateAddSize(4);
                        verts.buffer[size + 0] = new Vector3(v0x - slant, v0y);
                        verts.buffer[size + 1] = new Vector3(v0x + slant, v1y);
                        verts.buffer[size + 2] = new Vector3(v1x + slant, v1y);
                        verts.buffer[size + 3] = new Vector3(v1x - slant, v0y);
                    }
				}
				else // Bold
				{
					for (int j = 0; j < 4; ++j)
					{
                        ...
                        size = verts.size;
                        verts.SetAllocateAddSize(4);
                        verts.buffer[size + 0] = new Vector3(v0x + a - slant, v0y + b);
                        verts.buffer[size + 1] = new Vector3(v0x + a + slant, v1y + b);
                        verts.buffer[size + 2] = new Vector3(v1x + a + slant, v1y + b);
                        verts.buffer[size + 3] = new Vector3(v1x + a - slant, v0y + b);
                    }
				}
				...
				if (underline || strikethrough)
				{
					...
					if (uvs != null)
					{
						...
						for (int j = 0, jmax = (bold ? 4 : 1); j < jmax; ++j)
						{
                            ...
                            size = uvs.size;
                            uvs.SetAllocateAddSize(4);
                            uvs.buffer[size + 0] = new Vector3(cx, dash.u0.y);
                            uvs.buffer[size + 1] = new Vector3(cx, dash.u2.y);
                            uvs.buffer[size + 2] = new Vector3(cx, dash.u2.y);
                            uvs.buffer[size + 3] = new Vector3(cx, dash.u0.y);
                        }
					}
					...
					if (bold)
					{
						for (int j = 0; j < 4; ++j)
						{
                            ...
                            size = verts.size;
                            verts.SetAllocateAddSize(4);
                            verts.buffer[size + 0] = new Vector3(prevX + a, v0y + b);
                            verts.buffer[size + 1] = new Vector3(prevX + a, v1y + b);
                            verts.buffer[size + 2] = new Vector3(x + a, v1y + b);
                            verts.buffer[size + 3] = new Vector3(x + a, v0y + b);
                        }
					}
					else
					{
                        ...
                        size = verts.size;
                        verts.SetAllocateAddSize(4);
                        verts.buffer[size + 0] = new Vector3(prevX, v0y);
                        verts.buffer[size + 1] = new Vector3(prevX, v1y);
                        verts.buffer[size + 2] = new Vector3(x, v1y);
                        verts.buffer[size + 3] = new Vector3(x, v0y);
                    }
